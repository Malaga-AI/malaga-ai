...
export function Hero() {
  ...
  return (
    <section id="top" className="relative overflow-hidden py-20 md:py-28">
      ...
      <div className="absolute left-1/2 top-8 -z-10 h-72 w-72 -translate-x-1/2 rounded-full bg-teal-300/20 blur-3xl" />
      <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
        ...
        <motion.div
          className="relative min-h-[440px] overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.045] shadow-glow backdrop-blur"
        ...
        >
          ...
          <div className="absolute left-5 top-5 rounded-2xl border border-teal-300/20 bg-slate-950/75 p-4 sm:left-8 sm:top-8 sm:p-5">
            <p className="text-xs uppercase tracking-[0.24em] text-teal-200">Next session</p>
          ...
          {floating.map((item, index) => (
            <div
              key={item}
              className={`absolute ${nodePositions[index]} max-w-[9rem] rounded-full border border-white/12 bg-slate-950/70 px-3 py-2 text-center text-xs font-medium text-white backdrop-blur sm:px-4 sm:text-sm`}
            >
              {item}
            </div>
          ))}

          <div className="absolute inset-x-5 bottom-5 rounded-2xl border border-white/10 bg-slate-950/78 p-5 text-sm leading-6 text-slate-300 sm:inset-x-auto sm:right-8 sm:max-w-xs">
